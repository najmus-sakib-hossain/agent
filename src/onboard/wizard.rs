...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to DX — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 11, "Channels (How You Talk to DX)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("DX_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to DX)");
...
/// Use `dx onboard` or `dx onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `dx onboard --interactive` for the full wizard.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    if let Ok(custom_workspace) = std::env::var("DX_WORKSPACE") {
...
async fn run_quick_setup_with_home(
    ...
    let (dx_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    let config_path = dx_dir.join("config.toml");
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     dx agent -m \"Hello!\"");
            println!("    2. Gateway:  dx gateway");
            println!("    3. Status:   dx status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             dx auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              dx agent -m \"Hello!\"");
                println!("    3. Gateway:           dx gateway");
                println!("    4. Status:            dx status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.dx/config.toml");
            println!("    3. Chat:              dx agent -m \"Hello!\"");
            println!("    4. Gateway:           dx gateway");
        }
    } else {
        println!("    1. Chat:     dx agent -m \"Hello!\"");
        println!("    2. Gateway:  dx gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `dx models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: dx models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("DX works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        print_bullet("Available models: trinity-large-preview-free, big-pickle, minimax-m2.5-free");
        ...
        if key.trim().is_empty() {
            print_bullet(
                "No token provided. DX will open the GitHub device login flow on first use.",
            );
        }
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("DX will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how DX connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("DX uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("DX can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("DX can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should DX interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how DX stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_identity_backend() -> Result<IdentityConfig> {
    print_bullet("Choose the identity format DX should scaffold for this workspace.");
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to DX from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to DX from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to DX from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to DX from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("DX reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.dx/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("dx-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via DX_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— talk to DX from Lark or Feishu").dim()
                );
                print_bullet(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("DX will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if canonical_provider == "copilot" {
            ...
            println!("       {}", style("dx agent -m \"Hello!\"").yellow());
            println!(
                "       {}",
            ...
            println!(
                "       {}",
                style("dx auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: dx auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("dx channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("dx agent -m \"Hello, DX!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("dx status").yellow());
...
mod tests {
    ...
    async fn run_quick_setup_with_clean_env(
        ...
        let _workspace_env = EnvVarGuard::unset("DX_WORKSPACE");
        let _config_env = EnvVarGuard::unset("DX_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        tokio::fs::create_dir_all(&dx_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("dx-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".dx").join("config.toml");
        ...
        .await
        .expect("quick setup should honor DX_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** DX"),
            "should default agent name to DX"
        );
